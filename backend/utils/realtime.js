...
function emitToRoom(io, room, event, payload, { volatile = false } = {}) {
  ...
  log(`emit ${volatile ? '(volatile) ' : ''}${event} → ${room} listeners=${listeners} _id=${payload?._id || payload?.bookingId || ''}`)
}
...
function registerSocket(io) {
  ...
  io.on('connection', (socket) => {
    ...
    const employeeId = str(socket.handshake.auth?.employeeId || socket.handshake.query?.employeeId)
    const companyId  = str(socket.user?.companyId || socket.handshake.auth?.companyId || socket.handshake.query?.companyId)
...
/* utils */
function joinRoomSafe(socket, room) { if (!room) return; socket.join(room); socket.data.joined.add(room) }
function leaveRoomSafe(socket, room) { if (!room) return; socket.leave(room); socket.data.joined.delete(room) }
function listedRooms(socket) { return [...socket.rooms].filter((r) => r !== socket.id) }
function dumpRooms(socket, tag = '') { log(tag, socket.id, 'rooms=', listedRooms(socket)) }
function safeAck(ack, payload) { if (typeof ack === 'function') try { ack(payload) } catch (_) {} }
function normalizeSubPayload(p = {}) {
  return { role: str(p.role || ''), employeeId: str(p.employeeId || ''), companyId: str(p.companyId || ''), bookingId: str(p.bookingId || '') }
}
...
function validateJoin(socket, p) {
  ...
  // booking room: allow unless you add stricter checks
  return ''
}
