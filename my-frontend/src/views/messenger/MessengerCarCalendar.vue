...
/* ───────── Identity detection (same logic style as DriverCarBooking) ───────── */
...
function detectIdentity() {
  ...
  return found.find(x => x.loginId) || { loginId: '', role: 'MESSENGER' }
}
...
async function loadMonth() {
  ...
  try {
    ...
    console.error('[MessengerCarCalendar] loadMonth error:', e)
    loadError.value =
      e?.response?.data?.message || e?.message || 'Failed to load messenger tasks.'
    bookings.value = []
    showToast({
      type: 'error',
...
/* View in list page (MessengerCarBooking) */
function openListForSelected() {
  router.push({
    name: 'messenger-assignment',
...
<template>
  <div class="px-1 py-1 sm:px-0 text-slate-900 dark:text-slate-100">
    ...
    >
      <!-- Toolbar: same style family as DriverCarCalendar / EmployeeFoodCalendar -->
      <header
      ...
      >
        ...
        >
          <div class="flex items-start justify-between gap-2">
            <div>
              ...
              <p class="text-[11px] text-slate-500 dark:text-slate-400">
                {{ selectedList.length }}
                job{{ selectedList.length === 1 ? '' : 's' }} on this day
              </p>
            </div>
          ...
          >
            No messenger job on this date yet.
            Open the list page to see all of your tasks.
          </div>
