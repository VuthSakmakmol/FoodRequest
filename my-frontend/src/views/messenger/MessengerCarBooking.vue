...
const TEXT = {
  ...
  tableMessengerAck:{ en: 'Messenger',                     km: 'អ្នកបើកម៉ូតូ' },
...
/* responsive: simple breakpoint (no Vuetify) */
...
/* ─────────────── IDENTITY DETECTION (FORCE MESSENGER) ─────────────── */
...
/* Force role to MESSENGER for this view */
const identity = ref({
  ...detectIdentity(),
  role: 'MESSENGER',
})
...
const roleLabel = computed(() =>
  lang.value === 'km' ? 'អ្នកបើកម៉ូតូ' : 'Messenger'
)
...
const headers = computed(() => [
  ...
  { title: roleLabel.value,       key: 'messengerAck',width: 150, align: 'right' },
  { title: t('tableActions'),     key: 'actions',     width: 260, align: 'right' },
])

/* ─────────────── COLORS / ICONS (Tailwind + FA) ─────────────── */
const STATUS_BADGE_CLASS = {
  PENDING:   'bg-slate-100 text-slate-900 border-slate-400 dark:bg-slate-800 dark:text-slate-100 dark:border-slate-500',
  ASSIGNED:  'bg-sky-100 text-sky-900 border-sky-500 dark:bg-sky-900/40 dark:text-sky-100 dark:border-sky-500',
  ...
  DECLINED:  'bg-rose-100 text-rose-900 border-rose-500 dark:bg-rose-900/40 dark:text-rose-100 dark:border-rose-500',
}
...
async function loadList() {
  ...
  error.value   = ''
  try {
    const { loginId } = identity.value || { loginId: '' }

    const basePath = '/messenger/car-bookings'
    const params = { role: 'MESSENGER', loginId, messengerId: loginId }
...
function onCreated(doc) {
  ...
  const mine = myLogin && messengerId === myLogin
  if (!mine) return
...
async function onAssigned(p) {
  ...
  const newAssignee = String(p?.messengerId || '').toLowerCase()
  ...
  if (idx >= 0) {
    ...
    // if re-assigned to someone else, remove from my list
  ...
  // not in list yet, but now assigned to me → reload
  if (newAssignee && newAssignee === myLoginId) {
    await loadList()
  }
}
...
<template>
  <div class="px-1 py-1 sm:px-0">
    ...
    <div
      class="messenger-shell rounded-2xl border border-slate-300 bg-slate-100/80 shadow-sm
             dark:border-slate-700 dark:bg-slate-900/70"
    >
      <!-- HERO FILTER BAR -->
      <div
        class="flex flex-wrap items-end gap-3 border-b border-slate-400
               bg-gradient-to-r from-emerald-900 via-slate-800 to-sky-700
               px-3 py-3 text-slate-50 dark:border-slate-700"
      >
        <!-- Date -->
        <div class="flex-1 min-w-[140px] max-w-[180px]">
          ...
          <input
            ...
            class="w-full rounded-xl border border-sky-400 bg-sky-900/40 px-2.5 py-1.5
                   ...
                   dark:border-sky-500 dark:bg-slate-900/60"
          />
        </div>
      ...
      <div class="px-3 pb-3 pt-3">
        ...
        <div v-if="loading" class="space-y-2">
          <div class="h-10 w-full animate-pulse rounded-xl bg-slate-200/90 dark:bg-slate-700"></div>
          <div
            v-for="i in 3"
            :key="'sk-' + i"
            class="h-16 w-full animate-pulse rounded-xl bg-slate-200/80 dark:bg-slate-700/70"
          ></div>
        </div>

        <div v-else>
          <!-- MOBILE: CARD LIST -->
          <div v-if="isMobile" class="messenger-card-wrap">
            <p
              v-if="!paged.length"
            ...
            <div v-else class="messenger-card-list">
              <article
                v-for="item in paged"
                :key="item._id"
                :data-row-id="item._id"
                class="messenger-card rounded-2xl border border-slate-300 bg-white/95 p-3 text-xs shadow-[0_10px_24px_rgba(15,23,42,0.16)]
              ...
              >
                  ...
                  <div class="card-top-left">
                    <span
                      ...
                      :class="item.category === 'Car'
                        ...
                        : 'border-orange-500 bg-orange-50 text-orange-900 dark:border-orange-400 dark:bg-orange-900/30 dark:text-orange-100'"
                    >
                      <i
                        :class="item.category === 'Car'
                          ? 'fa-solid fa-car'
                  ...
                  <div class="val">
                    ...
                    <div v-if="item.ticketUrl" class="mt-1">
                      <a
                        ...
                        target="_blank"
                        rel="noopener"
                        class="inline-flex items-center gap-1 text-[11px] text-indigo-700 underline dark:text-indigo-300"
                      >
                ...
                <div class="card-row small">
                  ...
                  <div class="val">
                    <span
                      :class="ackBadgeClass(item.assignment?.messengerAck || 'PENDING')"
                    >
                      <i
                        :class="ackIconClass(item.assignment?.messengerAck || 'PENDING')"
                        class="text-[11px]"
                      />
                      <span>
                  ...
                  >
                    ...
                    <button
                      ...
                      class="inline-flex items-center gap-1 rounded-full border border-sky-500
                             ...
                             dark:border-sky-500 dark:bg-sky-900/30 dark:text-sky-100"
      ...
      >
        <div
          class="max-h-[90vh] w-full max-w-3xl overflow-hidden rounded-2xl border border-slate-300
                 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
        >
          <div class="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-700">
            ...
            <button
              type="button"
              class="inline-flex h-7 w-7 items-center justify-center rounded-full text-slate-500 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"
              @click="closeDetails"
            >
          ...
          <div class="max-h-[70vh] overflow-y-auto px-4 py-3 text-xs">
                ...
                >
                  ...
                  >
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener"
                      class="ml-1 inline-flex items-center gap-1 text-[11px] text-sky-700 underline dark:text-sky-300"
                    >
            ...
            <div class="mt-3 grid gap-3 md:grid-cols-2">
              <div>
                ...
                <div class="mt-1">
                  <span
                    :class="ackBadgeClass(detailItem?.assignment?.messengerAck || 'PENDING')"
                  >
                    <i
                      :class="ackIconClass(detailItem?.assignment?.messengerAck || 'PENDING')"
                      class="text-[11px]"
                    />
                    <span>
