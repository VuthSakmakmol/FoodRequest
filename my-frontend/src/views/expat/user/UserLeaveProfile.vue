...
async function fetchMyProfile(silent = false) {
  try {
    ...
    loadError.value =
      e?.response?.data?.message || 'Unable to load your leave balance.'
...
function setupRealtime() {
  ...
  offHandlers.push(
    onSocket('leave:req:created', p => { if (isMyDoc(p)) triggerRefresh('created') }),
    onSocket('leave:req:manager-decision', p => { if (isMyDoc(p)) triggerRefresh('manager') }),
    onSocket('leave:req:gm-decision', p => { if (isMyDoc(p)) triggerRefresh('gm') }),
    onSocket('leave:req:updated', p => { if (isMyDoc(p)) triggerRefresh('updated') })
  )
}
    ...
    >
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="min-w-[220px]">
          <p class="text-[10px] uppercase tracking-[0.25em] text-white/80">
            Expat Holiday
          </p>
      ...
      <div v-else>
        ...
        >
          <div class="flex items-center justify-between sm:block">
            ...
            <div>{{ profile?.employeeId || employeeId || '—' }}</div>
          </div>
        ...
        <div v-else class="overflow-x-auto">
          <table class="min-w-[620px] w-full text-left text-xs sm:text-[13px] text-slate-700 dark:text-slate-100">
            <thead
            ...
            <tbody>
              ...
              >
                ...
                <td class="table-td text-right">{{ b.yearlyEntitlement }}</td>
                <td class="table-td text-right">{{ b.used }}</td>
