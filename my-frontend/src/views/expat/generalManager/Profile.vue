...
async function fetchManagedList(silent = false) {
  try {
    ...
    // ✅ same endpoint, backend filters by LEAVE_GM -> gmLoginId
...
async function fetchEmployeeProfile(silent = false) {
  ...
  try {
    ...
    // ✅ SAME endpoint; GM passes employeeId as query
  ...
  <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
    ...
    <div class="px-2 pb-2 pt-3 sm:px-3 sm:pb-3">
      ...
      <div v-if="!isDetailMode">
        ...
        <!-- Desktop table (stable columns) -->
        <div v-else class="overflow-x-auto">
          <table
            class="min-w-[980px] w-full table-fixed text-left text-xs sm:text-[13px]
