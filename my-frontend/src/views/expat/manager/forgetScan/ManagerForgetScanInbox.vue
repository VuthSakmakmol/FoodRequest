...
async function confirmDecision() {
  ...
  try {
    await api.post(`/leave/forget-scan/${row._id}/manager-decision`, {
      action,
      comment: note,
      note,
      reason: note,
...
async function exportExcel() {
  try {
    ...
    const filename = `ForgetScan_ManagerInbox_${dayjs().format('YYYYMMDD_HHmm')}.xlsx`
        ...
        <div class="ui-hero-gradient">
          ...
          <div v-if="!isMobile" class="flex flex-wrap items-end justify-between gap-4">
            <div class="min-w-[240px]">
              <div class="text-[15px] font-extrabold">Manager Inbox · Forget Scan</div>
