<!-- src/views/expat/gm/GMProfile.vue
  ✅ One route does both:
     - /leave/gm/profile                 => list (gm-managed, approvalMode GM_ONLY)
     - /leave/gm/profile/:employeeId     => detail + PDF preview
-->
<script setup>
...
import TeamLeavePreviewModal from '@/views/expat/shared/components/TeamLeavePreviewModal.vue'

defineOptions({ name: 'GMProfile' })
...
const employeeIdParam = computed(() => s(route.params.employeeId))
const isDetail = computed(() => !!employeeIdParam.value)
...
async function fetchGmList() {
  ...
  try {
    ...
    console.error('fetchGmList error', e)
    listError.value = e?.response?.data?.message || e?.message || 'Failed to load GM staff profiles.'
    showToast({ type: 'error', title: 'Failed', message: listError.value })
  } finally {
...
function openEmployee(r) {
  const id = s(r?.employeeId)
  if (!id) return
  router.push({ name: 'leave-gm-profile', params: { employeeId: id } })
}
...
function normalizeProfileResponse(data) {
  ...
  const employeeId = s(p.employeeId || employeeIdParam.value || '')
...
async function fetchProfile() {
  if (!employeeIdParam.value) return
  loading.value = true
  error.value = ''
  try {
    const res = await api.get('/leave/user/profile', {
      params: { employeeId: employeeIdParam.value, ts: Date.now() },
    })
    me.value = normalizeProfileResponse(res?.data)
    updatedAt.value = s(res?.data?.meta?.updatedAt || new Date().toISOString())
  } catch (e) {
    console.error('fetchProfile error', e)
    error.value = e?.response?.data?.message || e?.message || 'Failed to load employee profile.'
...
function bindRealtime() {
  off.push(
    onSocket('leave:profile', (payload) => {
      ...
      if (!pid) return
      if (!isDetail.value) fetchGmList()
      if (isDetail.value && pid === me.value.employeeId) fetchProfile()
    }),
...
onMounted(async () => {
  bindRealtime()
  if (isDetail.value) await fetchProfile()
  else await fetchGmList()
})

watch(
  () => employeeIdParam.value,
  async () => {
    if (isDetail.value) await fetchProfile()
    else await fetchGmList()
  }
    ...
    <div class="flex items-start justify-between gap-3">
      <div>
        <div class="text-[22px] font-black">
          {{ isDetail ? 'Employee Leave Profile' : 'My GM Staff Profiles' }}
        </div>
        <div class="text-[12px] text-slate-400">
          GM view · {{ isDetail ? 'Profile & PDF Preview' : 'approvalMode = GM_ONLY only' }}
        </div>
      </div>

      <div class="flex items-center gap-2">
        <button v-if="isDetail" :class="[ui.btn, ui.btnGhost]" @click="router.push({ name: 'leave-gm-profile' })">
          <i class="fa-solid fa-list text-[11px]" />
          Staff List
        </button>
        <button :class="[ui.btn, ui.btnGhost]" :disabled="loading || listLoading" @click="isDetail ? fetchProfile() : fetchGmList()">
          <i class="fa-solid fa-rotate text-[11px]" :class="(loading || listLoading) ? 'fa-spin' : ''" />
          Refresh
        </button>
      </div>
    </div>
    ...
    <div v-if="!isDetail">
      <div class="mt-3 grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        <div :class="ui.card" class="p-4">
          ...
          <div class="text-[11px] text-slate-400">GM staff</div>
        </div>
      </div>

      <div v-if="listError" class="mt-3 rounded-xl border border-rose-700/50 bg-rose-950/30 px-4 py-3 text-[12px] text-rose-100">
        <b>Failed:</b> {{ listError }}
      </div>
          ...
          <table :class="ui.table">
            ...
            <tbody>
              <tr v-for="r in filtered" :key="r.employeeId" class="border-b border-slate-800 hover:bg-slate-950/40">
                <td :class="ui.td" class="font-mono font-bold">{{ r.employeeId }}</td>
                <td :class="ui.td" class="font-semibold">{{ r.name || '—' }}</td>
                <td :class="ui.td">{{ r.department || '—' }}</td>
                <td :class="ui.td" class="font-mono text-[11px] text-slate-300">
                  {{ ymd(r.contractDate) || '—' }} → {{ ymd(r.contractEndDate) || '—' }}
                </td>
                <td :class="ui.td" class="text-right">
                  <button :class="[ui.btn, ui.btnGhost]" @click="openEmployee(r)">
              ...
              <tr v-if="!filtered.length && !listLoading">
