...
/* Only real LEAVE_GM can make decisions */
const canGmDecide = computed(() =>
  auth.user?.role === 'LEAVE_GM'
)
...
/**
 * Tabs:
 * - PENDING_GM: show ALL rows for this GM (PENDING_GM + APPROVED/REJECTED/CANCELLED)
...
const filteredRows = computed(() => {
  ...
  // Sort by status then createdAt desc
  list.sort((a, b) => {
...
function openDecisionDialog(row, action) {
  // GM-only UI guard: admin and others are view-only
        ...
        >
          <div class="flex flex-col gap-1 min-w-[220px]">
            ...
            <p class="text-[11px] text-sky-50/90">
              Final approval queue for expatriate leave requests.
            </p>
          </div>
          ...
          <div class="flex flex-1 flex-wrap items-end justify-end gap-3">
            ...
            <div class="min-w-[220px] max-w-xs">
              <label
                class="mb-1 block text-[11px] font-medium text-sky-50"
              >
                Search
              </label>
              <div
            ...
            <button
              ...
              class="inline-flex items-center gap-1 rounded-full bg-white/95 px-3 py-1.5
                     text-[11px] font-semibold text-sky-700 shadow-sm hover:bg-slate-50
          ...
          >
            ...
            <dl class="mt-2 space-y-1.5">
              <div class="flex gap-2">
                <dt class="min-w-[80px] text-[11px] text-slate-500 dark:text-slate-400">
                  Date range
                </dt>
                <dd class="flex-1 text-[12px]">
                  {{ formatRange(row) }} ({{ row.totalDays }} days)
                </dd>
              </div>
...
/* Pagination buttons (same family as AdminLeaveTypes) */
.pagination-btn {
  padding: 4px 8px;
  border-radius: 999px;
