<!-- src/layouts/LeaveExpat/MGRLeaveExpat.vue
  ✅ SAME sidebar system as AdminLeaveExpat.vue (professional accordion groups)
  ✅ Desktop: collapsible (w-72 ↔ w-16)
  ✅ Mobile: overlay drawer (separate state, clean UX)
  ✅ Group active focus color + child active style
  ✅ Route-sync: keeps correct group opened
  ✅ ONE ToastContainer
-->
<script setup>
...
defineOptions({ name: 'MGRLeaveExpat' })
...
const groups = [
  {
    ...
    children: [
      ...
      { label: 'My Profile', icon: 'fa-solid fa-id-badge', to: { name: 'leave-manager-profile' } },
    ],
  },
]

/* Accordion open state (default open if any child is active) */
const open = reactive(
  Object.fromEntries(groups.map((g) => [g.key, g.children.some((c) => c.to?.name === route.name)]))
)

/* route sync: keep the active group opened */
watch(
  () => route.name,
  (name) => {
    groups.forEach((g) => {
...
const pageTitle = computed(() => route.meta?.title || 'Expat Leave Manager Portal')
    ...
    >
      ...
      <div class="flex items-center justify-between border-b px-2 py-2" style="border-color: rgb(var(--ui-border));">
        ...
        <div v-if="sidebarExpanded" class="min-w-0">
          ...
          <div class="truncate text-[10px] text-slate-500 dark:text-slate-400">Manager Portal</div>
        </div>
