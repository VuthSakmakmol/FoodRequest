<!-- src/layouts/LeaveExpat/UserLeaveExpat.vue
  ...
  ✅ No duplicate ToastContainer here (keep toast in parent layout if you already have one)
-->
<script setup>
...
defineOptions({ name: 'UserLeaveExpat' })
...
const groups = [
  {
    ...
    children: [
      ...
      // ✅ Replace Day (uncomment when routes ready)
...
<template>
  <div class="ui-page flex h-screen w-screen overflow-hidden">
    <!-- NOTE: No ToastContainer here (avoid duplicates). Put it in a parent layout if needed. -->
    ...
    >
      ...
      <div class="flex items-center justify-between border-b px-2 py-2" style="border-color: rgb(var(--ui-border));">
        ...
        <div v-if="sidebarExpanded" class="min-w-0">
          ...
          <div class="truncate text-[10px] text-slate-500 dark:text-slate-400">Employee Portal</div>
        </div>
        ...
        <div class="flex items-center gap-2 min-w-0">
          ...
          <div class="min-w-0">
            <div class="truncate text-[12px] font-extrabold text-slate-800 dark:text-slate-100">
              {{ route.meta?.title || 'Expat Leave Employee Portal' }}
            </div>
            <div class="truncate text-[10px] text-slate-500 dark:text-slate-400">
              {{ auth.user?.name || auth.user?.loginId || 'Employee' }}
            </div>
