...
const groups = [
  {
    ...
    children: [
      { label: 'Car Bookings', icon: 'fa-solid fa-clipboard-list', to: { name: 'driver-car-booking' } },
      { label: 'Calendar',     icon: 'fa-regular fa-calendar-days', to: { name: 'driver-carlendar' } },
    ],
  },
]
...
const appTitle = computed(() => route.meta?.title || 'Driver Panel')
      ...
      >
        ...
        >
          <div class="truncate font-semibold">
            {{ auth.user?.name || auth.user?.loginId || 'Driver' }}
          </div>
