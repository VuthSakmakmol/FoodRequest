...
/* Nav groups (Chef only handles Food) */
const groups = [
  {
    key: 'food',
    header: 'Food Request',
    ...
    children: [
      { label: 'Requests', icon: 'fa-solid fa-list-check',       to: { name: 'chef-requests' } },
      { label: 'Calendar', icon: 'fa-regular fa-calendar-days',  to: { name: 'chef-food-calendar' } },
    ],
  },
]
...
const appTitle = computed(() => route.meta?.title || 'Chef Panel')
      ...
      >
        ...
        >
          <div class="truncate font-semibold">
            {{ auth.user?.name || auth.user?.loginId || 'Chef' }}
          </div>
